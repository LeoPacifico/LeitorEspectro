...
def Contador(contador):
    if contador>10:
     print(f'NÚMERO DE PROCESSAMENTO: {contador} \nMÁXIMO DE PROCESSAMENTO ATINGIDO!\nENTRE EM CONTATO COM O ADM')
     print(":( :( :( :( :(")
...
def principal():
    ...
    if not exists("config.txt"):
        ...
        if not exists("espectro.txt"):
            ...
            input("\nProcessamento concluído. Pressione ENTER para novo processamento...")
